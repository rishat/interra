...
")/sm";
  ...
  function WackoMiddleprocess($things)
  {
    ...
    {
      return "\xa5\xa5<div align=\"center\">".preg_replace_callback($this->LONGREGEXP, $callback, $matches[1])."</div>\xa5\xa5";
    }
    return $thing;
  }
  ...
  function WackoCallback($things)
  {
    ...
    // urls
      ...
      || substr($url,-5)==".jpeg") return "<img src=\"".$matches[1]."\" alt=\"[image]\" />".$matches[2];
	...
	*/
    ...
    {
      $this->br = 0;
      return "<hr noshade=\"noshade\" size=\"1\" height=\"1\" />";
    }
    // forced line breaks
    //else if (preg_match("/^---\n?\s*$/", $thing, $matches))
    //{
             ...
             (preg_match("/^\(\((\S+)(\s+(.+))?\)\)$/", $thing, $matches)))
    {
      ...
      if ($url{0}=="*")
      {
       ...
       return ($sup?"<sup>":"")."<a href=\"".tREDURL."#o".$aname."\" name=\"".$aname."\">".$text."</a>".($sup?"</sup>":"");
      }
      else if ($url{0}=="#")
      {
       $anchor = substr($url, 1);
       ...
       if (!$text) $text = substr($url, 1);
       return ($sup?"<sup>":"")."<a href=\"".tREDURL."#".$ahref."\" name=\"o".$ahref."\">".$text."</a>".($sup?"</sup>":"");
      }
      else
      {
...
// define example customConfig class
 class CustomConfig extends WackoFormatterConfigDefault
 {
  ...
  { 
   ...
   /*
   else if (preg_match("/^(http|https|ftp|file):\/\/([^\\s\"<>]+)\.swf$/", $tag))
   {// flash movie 
     return "<img src=\"".$tag."\" ".($text?"alt=\"".$text."\" title=\"".$text."\"":"")." />";
   }
   */
   ...
   {// this is a valid external URL
     ...
     //figure out if this is a local link
		if(eregi("^".SERVER_ROOT,$url)){
			if(defined('INDIRECT_PATH')){
				$link = "[ <a href=\"$url\">&raquo;</a> ] <a href=\"".INDIRECT_PATH.$url."\" class=\"myLink\">$text</a>";
			}else{
				$link = "<a href=\"$url\" class=\"myLink\">$text</a>";
			}
		}else{
			if(defined('INDIRECT_PATH')){
				$link = "[ <a href=\"$url\" target=\"_blank\">&raquo;</a> ] <a href=\"".INDIRECT_PATH.$url."\" class=\"myLink\" target=\"_blank\">$text</a>";
			}else{
				$link = "<a href=\"$url\" class=\"myLink\" target=\"_blank\">$text</a>";	
			}
		}
     return $link; 
   }else if(preg_match("/^ljuser$/",$tag)){
	    $link = "<nobr><a href=\"http://www.livejournal.com/users/".$text."/info/\" target=\"_blank\"><img src=\"".SERVER_ROOT . TEMPLATE_ROOT . "images/user.gif\" alt=\"[info]\" style=\"border: 0pt none ; vertical-align: center;\" /></a><a href=\"http://www.livejournal.com/users/".$text."/\" target=\"_blank\"><b>".$text."</b></a></nobr>";
  ...
  { 
   $fullfilename = "lib/nwacko/".$formatter.".php";
